# Making new GR4J model - introducing x5, x6, x7 for new ET equation
# Rory Smith
# 27th April 2025
...
#'
#' @name gr7jt
#' @aliases gr7jt.sim gr4jrouting gr4jrouting.sim
...
#' @param x5 'a' coefficient of new ET equation.
#' @param x6 'b' coefficient of new ET equation.
#' @param x7 'c' coefficient of new ET equation.
...
gr7jt.sim <-
  function(DATA,
    ...
    ## check values, a sanity check
    stopifnot(x1 >= 0, x5 >= 0, x6 >= 0, x7 >= 0)
    stopifnot(etmult >= 0)
    stopifnot(S_0 >= 0)
    stopifnot(S_0 <= 1)
    ...
    DATA <- as.ts(DATA)
    P <- DATA[, "P"]
    E <- DATA[, "E"] * etmult    #multiplies E by etmult for calibration purposes
    ...
    if (COMPILED) {
      ...
      for (t in seq(1, length(P))) {
        Pn <- max(P[t] - E[t], 0)    # net rainfall
        En <- max(E[t] - P[t], 0)    # net evaporative demand
        St_x1 <- S_prev / x1         # relative soil fill (0 to 1)
        ## production
        Ps <- 0
        ET[t] <- 0
        if (Pn > 0) {
          ...
          ## Actual evapotranspiration when demand exceeds rain
          
          # Original ET equation in GR4J
          # ET[t] <- (S_prev * (2 - St_x1) * tanh(En / x1) /
                      # (1 + (1 - St_x1) * tanh(En / x1)))
          
          # Custom ET equation with calibration variables - Rory
          ET[t] <- En * ((St_x1)^x5 - x6*(1 - (1 - St_x1)^x7))
          ET[t] <- min(ET[t], S_prev)
        }
        ...
        S[t] <- S[t] - perc         # update production store after percolation
        U[t] <- perc + (Pn - Ps)    # effective rainfall that feeds routing (matches diagram)
        S_prev <- S[t]              # update storage
      }
    }
...
## used for calibration bounds/defaults
gr7jt.ranges <- function() {
  list(
    x1 = c(100, 1200),
    x5 = c(0.01, 1),
    x6 = c(0.01, 1),
